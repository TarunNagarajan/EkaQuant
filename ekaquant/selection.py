...
def _valid_scores(
    sensitivity_map: Dict[str, float], arg_name: str = "sensitivity_map"
) -> np.ndarray:
...
def threshold_elbow(sensitivity_map: Dict[str, float]) -> float:
    ...
    kneedle = KneeLocator(
        range(len(scores)), scores, curve="convex", direction="decreasing"
    )
...
def threshold_gradient(
    sensitivity_map: Dict[str, float], sensitivity_ratio: float = 0.01
) -> float:
...
def threshold_cumulative(
    sensitivity_map: Dict[str, float], budget: float = 0.95
) -> float:
    if not (0 < budget <= 1):
...
def knapsack_keep_layers(
    model, sensitivity_map: Dict[str, float], budget_mb: float
) -> List[str]:
    items = []
    for name, module in model.named_modules():
        if not isinstance(module, nn.Linear) or name not in sensitivity_map:
            continue
        cost_keep = get_module_cost_mb(module, "fp16") - get_module_cost_mb(
            module, "int4"
        )
