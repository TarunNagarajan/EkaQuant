...
run_realtime_cmd("pip install -q transformers bitsandbytes accelerate peft datasets numpy scipy kneed scikit-image tqdm evaluate rouge_score pandas tabulate")
...
run_realtime_cmd(f"sed -i 's/indic\\.mmlu_in\\.evaluate_mmlu_in/multilingual.mmlu_in.evaluate_mmlu_in/g' {CONFIG_PATH}")
...
def run_single_eval(model_id, precision):
    ...
    if precision == 8:
        ...
        run_realtime_cmd(f"sed -i 's/load_in_4bit *= *True/load_in_8bit=True/g' {LOADER_PATH}")
        run_realtime_cmd(f"sed -i 's/load_in_8bit *= *True/load_in_8bit=True/g' {LOADER_PATH}")
    else:
        print("Switching model_loader to 4-bit...")
        run_realtime_cmd(f"sed -i 's/load_in_8bit *= *True/load_in_4bit=True/g' {LOADER_PATH}")
        run_realtime_cmd(f"sed -i 's/load_in_4bit *= *True/load_in_4bit=True/g' {LOADER_PATH}")
    ...
    rc = run_realtime_cmd("python eka-eval/scripts/run_benchmarks.py", input_str=wizard_input)
