...
def setup_pristine_environment():
    ...
    if os.path.exists(config_path):
        ...
        config_content = config_content.replace(
            "indic.mmlu_in.evaluate_mmlu_in", "multilingual.mmlu_in.evaluate_mmlu_in"
        )
        config_content = config_content.replace(
            "indic.arc_c_in.evaluate_arc_c_in",
            "multilingual.arc_c_in.evaluate_arc_c_in",
        )
        config_content = config_content.replace(
            '"save_detailed": False', '"save_detailed": True'
        )
...
def run_evaluation():
    ...
    if source_dir:
        ...
        shutil.copy2(
            os.path.join(source_dir, "calculated.csv"),
            os.path.join(abs_folder, "calculated.csv"),
        )
