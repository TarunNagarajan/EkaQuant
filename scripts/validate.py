...
def main():
    parser = argparse.ArgumentParser(
        description="Validate KL Divergence of EkaQuant vs Uniform 4-bit"
    )
    parser.add_argument(
    ...
    model_eka = quantizer.quantize(
        calibration_texts=[], selection_method="knapsack", budget_mb=args.budget_mb
    )
    ...
    for i, text in enumerate(test_texts):
        print(
            f"Text {i + 1}: Uniform KL: {uniform_kls[i]:.6f} | EkaQuant KL: {eka_kls[i]:.6f}"
        )
